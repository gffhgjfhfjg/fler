...
7. **非 root 动态分析**：目标 App 所在设备无 root 时，用 `gadget_repack_apk(apkPath=…, mode=…)` 把 frida-gadget 重打包注入任意 APK（libgadget 三件套 + 入口 clinit 注入 + extractNativeLibs + 重签名）。`mode=listen`（默认）装好后启动目标 App 用 frida 连 `127.0.0.1:27042`（进程名 Gadget，`onLoad=wait` 时启动冻结等待 attach）；`mode=script` 启动时自动跑签名伪装/完整性指纹对抗（重打包签名变化与 dex/manifest 改动的自校验）。产物同 export 下载，安装前同样需卸载原 App。
